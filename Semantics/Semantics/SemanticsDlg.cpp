...
DWORD WINAPI AutomateWordByImport(vector <EntityBase *> * all_entities, TCHAR * docFileName)
{
	...
	try
	{
		...
		Word::TablePtr spTable = spTables->Add(spParaRng, all_entities->size() + 1, 7, &par, &par);
//		spTable->OleFunction("Cell", 1, 1)->OlePropertyGet("Range")->OlePropertySet("Text", "Столбец 1, строка 1");
		...
		spTable->Cell(1, 7)->Range->InsertAfter(L"Перевод на естественный язык");
...
void CSemanticsDlg::OnBnClickedSavescheme()
{
	...
	for ( size_t i = 0; i < all_entities.size(); i ++)
	{
		...
		else  // если формула
		{
			...
			for ( size_t i1 = 0; i1 < pf->getNumberOfParents(); i1++)
			{
				ParentLink * ppl = pf->getParentLink(i1);
...
void CSemanticsDlg::OnBnClickedLoadscheme()
{
	...
	CFileDialog fileDialog(FALSE, strExt, NULL, OFN_ENABLESIZING | OFN_PATHMUSTEXIST | OFN_FILEMUSTEXIST,
							strFilter, this);
	fileDialog.m_ofn.lpstrTitle = strTitle;
